...
;(define-syntax-rule (set-cdr! lst val)
;  (if (not (list? lst)) lst
...
(define (list-of-values exps env)
  (if (no-operands? exps)
      '()
      (cons (eval-interp (first-operand exps) env)
            (list-of-values (rest-operands exps) env))))

;(define (eval-if exp env)
;  (if (true? (eval-interp (if-predicate exp) env))
;      (eval-interp (if-consequent exp) env)
;      (eval-interp (if-alternative exp) env)))

(define (eval-sequence exps env)
  (cond ((last-exp? exps) (eval-interp (first-exp exps) env))
        (else (eval-interp (first-exp exps) env)
              (eval-sequence (rest-exps exps) env))))

(define (eval-assignment exp env)
  (set-variable-value! (assignment-variable exp)
                       (eval-interp (assignment-value exp) env)
                       env)
  'ok)
...
;  (let ((input (read)))
;    (let ((output (eval-interp input the-global-environment)))
...
(define (eval-interp exp env)
  (cond ((self-evaluating? exp) exp)
        ...
        ((application? exp)             ; clause from book
         (apply-interp (actual-value (operator exp) env)
                (operands exp)
                env))
        (else
         (error "Unknown expression type -- EVAL" exp))))
...
(define (actual-value exp env)
  (force-it (eval-interp exp env)))

(define (apply-interp procedure arguments env)
  (cond ((primitive-procedure? procedure)
